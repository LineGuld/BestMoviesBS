...
namespace BestMoviesBS.DataAccess
{
    public class UserDao : IUserDao
    {
        ...
        public async Task<User> NewUserCheck(string userId)
        {
            var query = $@"
            MATCH (u:User {{id: $id}})
            ...
            try
            {
                ...
                foreach (var result in readResults)
                {
                    // If the user is not present in the database, return him/her
        ...
        public async Task AddNewUser(string userId, string userName)
        {
            _user = await NewUserCheck(userId);
            ...
            CREATE (u:User {{id: $id, username: $username}})";
                ...
                try
                {
                    var writeResults = await session.ExecuteWriteAsync(async tx =>
                    {
                        var result = await tx.RunAsync(query, new {id = userId, username = userName});
                        return await result.ToListAsync();
                    });

                    foreach (var result in writeResults)
                    {
                        Console.WriteLine("Added user with username " + result["u.username"].As<String>() + " and id " +
                                          result["u.id"]);
                    }
                }
                catch (Neo4jException ex)
